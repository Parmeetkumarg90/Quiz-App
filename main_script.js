...
function addQues() {
    ...
    ques.innerHTML = `<span>Ques ${new_index + 1} :</span>${quizQues[index].question}`;
    let opt = document.querySelectorAll('.opt');
    opt.forEach(b => b.classList.remove("selected"));
    opt[0].innerHTML = `<input type="radio" id="option1" value="${quizQues[index].options[0]}" name="option"> ${quizQues[index].options[0]}`;
    opt[1].innerHTML = `<input type="radio" id="option2" value="${quizQues[index].options[1]}" name="option"> ${quizQues[index].options[1]}`;
    opt[2].innerHTML = `<input type="radio" id="option3" value="${quizQues[index].options[2]}" name="option"> ${quizQues[index].options[2]}`;
    opt[3].innerHTML = `<input type="radio" id="option4" value="${quizQues[index].options[3]}" name="option"> ${quizQues[index].options[3]}`;
    let button = document.querySelector('.buttons');
    button.innerHTML = '<button id="submit">Submit</button><button id="next">Next</button>';
...
function report() {    
    document.querySelectorAll('.box')[0].style.display = 'none';
    document.querySelectorAll('.box')[1].style.display = 'none';
    document.querySelectorAll('.box')[2].style.display = 'grid';
    let ul = document.querySelectorAll('ul')[1];
    let li = ul.querySelectorAll('li');
    li[0].innerText = `Total Questions : ${quizQues.length}`;
    li[1].innerText = `Attempted Questions : ${totalAtempt}`;
    li[2].innerText = `Total Corrected Questions : ${totalCorrect}`;
    li[3].innerText = `Total Wrong Qestions : ${totalAtempt - totalCorrect}`;
    ...
    document.querySelectorAll('.box')[2].appendChild(div);    
    startButton = document.querySelectorAll('.startButton')[1];
    start(startButton);
}
function buttons() {
    ...
    submit.addEventListener('click', (event) => {
        if (event.isTrusted) {
            if (document.querySelector('input[name="option"]:checked')) {
                clearInterval(time1);
                askQues[new_index-1].select = document.querySelector('input[name="option"]:checked').value;
                calculate(document.querySelector('input[name="option"]:checked').value, index);
                addQues();
            }
            else {
...
function calculate(option, quesNo) {
    console.log(quizQues[quesNo])
    if (quizQues[quesNo].answer === option) {
        totalCorrect++;
    }
    totalAtempt++;
}
